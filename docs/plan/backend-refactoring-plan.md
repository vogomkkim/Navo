...
#### 3차 (품질): 테스트(e)
- ✅ **완료**: `ai`, `auth`, `static`
- 🔄 **진행 중**: `agents`, `events` (테스트 작성 완료, 실행 검증 필요)
- ⏳ **대기**: `health`, `db`, `projects`, `components`, `analytics`, `pages`
...
**3차 우선순위 완료 후**: 4차 우선순위로 넘어가기 전에 남은 모듈들의 1차, 2차 완료
1. **components 모듈**: ✅ 1차, 2차 완료
2. **analytics 모듈**: ✅ 1차, 2차 완료
3. **pages 모듈**: ✅ 1차, 2차 완료

**모든 모듈 1-3차 완료 후**: 4차 우선순위 진행
1. **import 경로 전환**: 기존 코드에서 새 모듈 경로로 점진적 전환
2. **soft deprecate**: 레거시 코드 제거
