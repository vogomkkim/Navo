...
describe('AuthService', () => {
  ...
  describe('register', () => {
    it('이미 존재하는 이메일로 회원가입 시 에러를 발생시켜야 한다', async () => {
      ...
      const registerData = { email: 'mk.kim@vogoplay.com', password: 'password123' };
      ...
      expect(mockAuthRepository.findUserByEmail).toHaveBeenCalledWith('mk.kim@vogoplay.com');
    ...
    it('새로운 사용자는 성공적으로 회원가입되어야 한다', async () => {
      ...
      const registerData = { email: 'new@example.com', password: 'newpassword' };
  ...
  describe('login', () => {
    it('존재하지 않는 사용자로 로그인 시 에러를 발생시켜야 한다', async () => {
      ...
      await expect(authService.login(loginData)).rejects.toHaveProperty('errorCode', 'INVALID_CREDENTIALS');
      expect(mockAuthRepository.findUserByEmail).toHaveBeenCalledWith('nonexistent@example.com');
    });
    ...
    it('잘못된 비밀번호로 로그인 시 에러를 발생시켜야 한다', async () => {
      ...
      verifyPasswordMock.mockResolvedValueOnce({ ok: false } as passwordModule.VerifyResult);
