...
export default async function eventRoutes(
  ...
  fastify.post('/events', { preHandler: [fastify.authenticateToken] }, async (request, reply) => {
    try {
      ...
      if (eventsArray && Array.isArray(eventsArray)) {
        ...
        for (const event of eventsArray) {
          if (!event.type) {
            reply.status(400).send({ error: '모든 이벤트에 타입이 필요합니다.' });
            return;
          }
        }

        const count = await eventsService.storeUserEvents(eventsArray, userId);
  ...
  fastify.post('/events/log-error', { preHandler: [fastify.authenticateToken] }, async (request, reply) => {
    try {
      const userId = (request as any).userId as string | undefined;
      if (!userId) {
        reply.status(401).send({ error: '사용자 인증이 필요합니다.' });
      ...
      reply.status(400).send({
        error: '잘못된 에러 형식입니다. message, filename, stack 중 하나는 필요합니다.',
      });
    } catch (error) {
  ...
  // Get user events
  fastify.get('/events/user', { preHandler: [fastify.authenticateToken] }, async (request, reply) => {
    try {
      const userId = (request as any).userId as string | undefined;
      if (!userId) {
        reply.status(401).send({ error: '사용자 인증이 필요합니다.' });
  ...
  // Get project events
  fastify.get('/events/project/:projectId', { preHandler: [fastify.authenticateToken] }, async (request, reply) => {
    try {
      const userId = (request as any).userId as string | undefined;
      if (!userId) {
        reply.status(401).send({ error: '사용자 인증이 필요합니다.' });
