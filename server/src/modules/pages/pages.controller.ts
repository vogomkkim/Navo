...
export function pagesController(app: FastifyInstance) {
    ...
    app.get('/api/pages/project/:projectId', {
    ...
    // Get page by path
    app.get('/api/pages/project/:projectId/path/*', {
        preHandler: [app.authenticateToken]
    }, async (request, reply) => {
        try {
            const userId = (request as any).userId as string | undefined;
    ...
    app.patch('/api/pages/:pageId', {
        ...
        try {
            ...
            if (body.description !== undefined) pageData.description = body.description;
            if (body.layoutJson !== undefined) pageData.layoutJson = body.layoutJson;
            if (body.isPublished !== undefined) pageData.isPublished = body.isPublished;
    ...
    // Publish page
    app.post('/api/pages/:pageId/publish', {
        preHandler: [app.authenticateToken]
    }, async (request, reply) => {
        try {
            const userId = (request as any).userId as string | undefined;
    ...
    // Unpublish page
    app.post('/api/pages/:pageId/unpublish', {
        preHandler: [app.authenticateToken]
    }, async (request, reply) => {
        try {
            const userId = (request as any).userId as string | undefined;
