...
import { ProjectsRepositoryImpl, Project, ProjectPage } from './projects.repository';
...
export class ProjectsService {
    ...
    async listProjectPages(projectId: string, userId: string): Promise<ProjectPage[]> {
        try {
            // Verify project ownership first
            const project = await this.repository.getProjectByUserId(projectId, userId);
            if (!project) {
    ...
    async renameProject(projectId: string, name: string, userId: string): Promise<Project> {
        try {
            // Verify project ownership first
            const project = await this.repository.getProjectByUserId(projectId, userId);
            if (!project) {
