...
export class VfsRepositoryImpl implements VfsRepository {
  ...
  private async ensureRootNodeExists(
    projectId: string,
    tx: any
  ): Promise<VfsNode> {
    ...
    this.app.log.warn(
      `Root node for project ${projectId} not found. Creating it defensively.`
    );
  ...
  async syncArchitecture(
    ...
    try {
      await db.transaction(async (tx) => {
        ...
        const createNodesRecursively = async (
          ...
          for (const node of nodes) {
            ...
            if (content && content.length > MAX_FILE_SIZE_BYTES) {
              throw new Error(
                `File content for "${node.name}" exceeds the limit of 1MB.`
              );
            }
        ...
        let nodesToCreate =
          (architecture as any).structure ||
          (architecture as any).file_structure;
        if (nodesToCreate) {
  ...
  async listNodesByParentId(
    ...
    try {
      const whereCondition = parentId
        ...
        : and(eq(vfsNodes.projectId, projectId), isNull(vfsNodes.parentId));
      const dbRows = await db
        .select()
        .from(vfsNodes)
        .where(whereCondition)
        .orderBy(vfsNodes.nodeType, vfsNodes.name);
  ...
  async listNodesByProject(
    ...
    try {
      ...
      return (result as any).rows as VfsNode[];
    } catch (error) {
      this.app.log.error(
        { error, paths },
  ...
  async createNode(params: {
    ...
    try {
      ...
      if (
        ...
        error.message.includes("vfs_nodes_project_id_parent_id_name_key")
      ) {
        throw new Error(
          "A node with the same name already exists in this directory."
        );
      }
      ...
      for (const segment of segments) {
        const rows = await tx
          ...
          .where(
            and(
              eq(vfsNodes.projectId, projectId),
              eq(vfsNodes.parentId, parentNode.id),
              eq(vfsNodes.name, segment)
            )
          )
          .limit(1);
  ...
  async findOrCreateByPath(
    ...
    this.app.log.info(
      `[findOrCreateByPath] Called for project ${projectId} with path "${path}" (Normalized: "${normalizedPath}")`
    );

    if (normalizedPath === "") {
      this.app.log.info(
        `[findOrCreateByPath] Path is root, ensuring root node exists.`
      );
      return this.ensureRootNodeExists(projectId, transaction);
    }

    const existingNode = await this.findByPath(
      projectId,
      normalizedPath,
      transaction
    );
    if (existingNode) {
      this.app.log.info(
        `[findOrCreateByPath] Node already exists at "${normalizedPath}". Returning existing node.`
      );
      return existingNode;
    }
    ...
    this.app.log.info(
      { parentNode: { id: parentNode.id, name: parentNode.name } },
      `[findOrCreateByPath] Initial parent node (root)`
    );
    ...
    for (const segment of segments) {
      ...
      if (!childNode) {
        ...
        const nodeType =
          !isDirectoryPath && isLastSegment ? "FILE" : "DIRECTORY";

        this.app.log.info(
          { segment, parentId: parentNode.id, nodeType },
          `[findOrCreateByPath] Node not found. Creating new node.`
        );
        ...
        this.app.log.info(
          { segment, parentId: parentNode.id },
          `[findOrCreateByPath] Found existing child node.`
        );
      }
      parentNode = childNode;
      this.app.log.info(
        { parentNode: { id: parentNode.id, name: parentNode.name } },
        `[findOrCreateByPath] New parent for next iteration`
      );
    }

    this.app.log.info(
      { finalNode: { id: parentNode.id, name: parentNode.name } },
      `[findOrCreateByPath] Finished. Returning final node.`
    );
    return parentNode;
  }
  ...
  async upsertByPath(
    ...
    return db.transaction(async (tx) => {
      ...
      // ... (rest of the logic)
      return await this.findOrCreateByPath(projectId, path, tx);
    });
  }
}
