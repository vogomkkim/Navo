...
export function projectsController(app: FastifyInstance) {
    ...
    // List project pages
    app.get('/api/projects/:projectId/pages', {
        preHandler: [app.authenticateToken]
    }, async (request, reply) => {
        try {
            const userId = (request as any).userId as string | undefined;
            ...
            reply.status(500).send({ error: '프로젝트 페이지 목록 조회에 실패했습니다.' });
        }
    });
    ...
    app.patch('/api/projects/:projectId', {
        ...
        try {
            ...
            if (!name || typeof name !== 'string' || name.trim().length < 2) {
                reply.status(400).send({ error: '유효하지 않은 이름입니다. 최소 2자 이상 입력해주세요.' });
                return;
            }

            const project = await projectsService.renameProject(projectId, name, userId);
            reply.send({ project });
        } catch (error) {
    ...
    // Rollback project
    app.post('/api/projects/:projectId/rollback', {
        preHandler: [app.authenticateToken]
    }, async (request, reply) => {
        try {
            const userId = (request as any).userId as string | undefined;
