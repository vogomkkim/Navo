...
export const vfsNodes = pgTable(
  "vfs_nodes",
  {
    ...
    parentId: uuid("parent_id").references((): AnyPgColumn => vfsNodes.id, {
      // Use AnyPgColumn for self-reference
      onDelete: "cascade",
    }),
...
export const chatMessagePayloads = pgTable("chat_message_payloads", {
...
export const integrations = pgTable(
  "integrations",
  {
    ...
    userId: uuid("user_id")
      .notNull()
      .references(() => users.id, { onDelete: "cascade" }),
    serviceType: varchar("service_type", { length: 50 }).notNull(), // 'vercel', 'github', 'aws', etc.
    credentials: jsonb("credentials").notNull(), // 암호화된 토큰/키
    metadata: jsonb("metadata"), // 서비스별 추가 정보
  ...
  (table) => [
    index("idx_integrations_user_id").using("btree", table.userId),
    index("idx_integrations_service_type").using("btree", table.serviceType),
    unique("unique_user_service").on(table.userId, table.serviceType), // 사용자당 서비스별 하나만
  ]
);
