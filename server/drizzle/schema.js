...
export const componentDefinitions = pgTable('component_definitions', {
    ...
    index('idx_component_definitions_project').using('btree', table.projectId.asc().nullsLast().op('uuid_ops')),
    foreignKey({
        columns: [table.projectId],
        foreignColumns: [projects.id],
        name: 'component_definitions_project_id_fkey',
    }).onDelete('cascade'),
    unique('component_definitions_project_id_name_key').on(table.projectId, table.name),
]);
export const projects = pgTable('projects', {
    ...
    index('idx_projects_owner').using('btree', table.ownerId.asc().nullsLast().op('uuid_ops')),
    foreignKey({
...
export const pages = pgTable('pages', {
    ...
    index('idx_pages_project').using('btree', table.projectId.asc().nullsLast().op('uuid_ops')),
    foreignKey({
        columns: [table.projectId],
        foreignColumns: [projects.id],
...
export const components = pgTable('components', {
    ...
    index('idx_components_definition').using('btree', table.componentDefinitionId.asc().nullsLast().op('uuid_ops')),
    index('idx_components_page_order').using('btree', table.pageId.asc().nullsLast().op('int4_ops'), table.orderIndex.asc().nullsLast().op('int4_ops')),
    foreignKey({
...
export const chatMessages = pgTable('chat_messages', {
    ...
    index('idx_chat_messages_session').using('btree', table.sessionId.asc().nullsLast().op('uuid_ops')),
    index('idx_chat_messages_user').using('btree', table.userId.asc().nullsLast().op('uuid_ops')),
    foreignKey({
...
export const projectPlans = pgTable('project_plans', {
    ...
    index('idx_project_plans_project').using('btree', table.projectId.asc().nullsLast().op('uuid_ops')),
    index('idx_project_plans_user').using('btree', table.userId.asc().nullsLast().op('uuid_ops')),
    index('idx_project_plans_status').using('btree', table.status.asc().nullsLast().op('text_ops')),
    foreignKey({
        columns: [table.projectId],
        foreignColumns: [projects.id],
...
export const virtualPreviews = pgTable('virtual_previews', {
    id: uuid().defaultRandom().primaryKey().notNull(),
    pageId: uuid('page_id').notNull(),
    ...
    index('idx_virtual_previews_page').using('btree', table.pageId.asc().nullsLast().op('uuid_ops')),
    index('idx_virtual_previews_file_path').using('btree', table.filePath.asc().nullsLast().op('text_ops')),
    foreignKey({
        columns: [table.pageId],
        foreignColumns: [pages.id],
