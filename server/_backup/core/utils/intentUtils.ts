...
const ALLOWED_SCOPES: TargetScope[] = [
  'project',
  'page',
  'component',
  'code',
...
export function normalizeModelAnalysis(obj: any): ModelAnalysis {
  ...
  const description =
    typeof obj?.description === 'string' && obj.description.length > 0
      ...
      : '의도 분석 결과';
  const is_vague =
    typeof obj?.is_vague === 'boolean' ? obj.is_vague : !!obj?.isVague;

  const targets: TargetMeta[] =
    Array.isArray(obj?.targets) && obj.targets.length > 0
  ...
  const routing_key: string =
    typeof obj?.routing_key === 'string' ? obj.routing_key : '';
  const enhanced_message: string =
    typeof obj?.enhanced_message === 'string' ? obj.enhanced_message : '';
...
export function decideExecution(ia: ModelAnalysis): ExecutionDecision {
  ...
  if (!(['page', 'component'] as TargetScope[]).includes(scope))
    blocks.push('unsafe_scope');

  if (blocks.length) {
    return {
      status: 'blocked',
      reason: blocks.join(','),
      missing: ia.required_fields,
    };
  }
