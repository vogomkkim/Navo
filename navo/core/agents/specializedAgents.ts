...
export class ProjectCreationAgent implements Agent {
    ...
    async execute(
        ...
        try {
            ...
            // 기능별 페이지 생성
            if (projectData.features.includes('자기소개') || projectData.features.includes('소개')) {
                pagesToCreate.push({
                    name: '소개',
                    path: '/about',
                    layoutJson: {
            ...
            if (projectData.features.includes('프로젝트 갤러리') || projectData.features.includes('프로젝트')) {
                pagesToCreate.push({
                    name: '프로젝트',
                    path: '/projects',
                    layoutJson: {
            ...
            if (projectData.features.includes('연락처') || projectData.features.includes('contact')) {
                pagesToCreate.push({
                    name: '연락처',
                    path: '/contact',
                    layoutJson: {
...
export class DevelopmentSetupAgent implements Agent {
    ...
    canHandle(intent: string): boolean {
        ...
        console.log(`💻 DevelopmentSetupAgent.canHandle("${intent}"): ${canHandle}`);
        return canHandle;
    }
...
export class ComponentModificationAgent implements Agent {
    ...
    canHandle(intent: string): boolean {
        ...
        console.log(`🧩 ComponentModificationAgent.canHandle("${intent}"): ${canHandle}`);
        return canHandle;
    }
...
export class GeneralConversationAgent implements Agent {
    ...
    canHandle(intent: string): boolean {
        ...
        console.log(`💬 GeneralConversationAgent.canHandle("${intent}"): ${canHandle}`);
        return canHandle;
    }
