...
async function loadInitialData() {
  ...
  try {
    ...
    if (ui.canvasEl)
      ui.canvasEl.innerHTML = DOMPurify.sanitize(
        renderLayout(currentLayout as any)
      );
    if (ui.infoEl)
...
function addEventListeners() {
  ...
  if (ui.logoutBtnMobile)
    ui.logoutBtnMobile.addEventListener('click', handleLogout);
  ...
  if (ui.canvasEl)
    ui.canvasEl.addEventListener('click', (ev) => {
      ...
      if (componentEl) {
        const componentId = (componentEl as any).dataset.id as
          | string
          | undefined;
        const component =
          currentLayout && componentId
            ? currentLayout.components.find((c) => c.id === componentId)
            : undefined;
        track({
...
function handleTextEdit(element: HTMLElement) {
  ...
  const finishEditing = () => {
    ...
    const componentId = (element as any).dataset?.componentId as
      | string
      | undefined;
