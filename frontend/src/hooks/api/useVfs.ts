...
interface RenameVfsNodePayload {
  ...
  nodeId: string;
  name: string;
  parentId: string | null; // Needed for precise cache invalidation
}
  ...
  return useMutation<VfsNodeResponse, Error, CreateVfsNodePayload>({
    ...
    onSuccess: (data, variables) => {
      queryClient.invalidateQueries({
        queryKey: ['vfsNodes', variables.projectId, variables.parentId],
      });
    },
    ...options,
  });
...
export function useRenameVfsNode(
  ...
  return useMutation<VfsNodeResponse, Error, RenameVfsNodePayload>({
    ...
    onSuccess: (data, variables) => {
      ...
      // Also update the content cache if it exists
      queryClient.setQueryData(['vfsNode', variables.projectId, variables.nodeId], (oldData: any) =>
        oldData ? { ...oldData, node: { ...oldData.node, name: variables.name } } : oldData
      );
    },
    ...options,
  });
...
export function useDeleteVfsNode(
  ...
  const { token, logout } = useAuth();
  const queryClient = useQueryClient();
  const closeOpenFile = useIdeStore((state) => state.closeOpenFile);
